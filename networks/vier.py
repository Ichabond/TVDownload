...
class Vier(networks.ClosedTVNetwork):
        ...
        for link in soup.find_all("div", attrs={'class': "video-playlist-slideshow-block"}):
            if link.h1 is not None:
                if name.lower() in link.h1.string.lower():
                    ...
                    return "http://vier.be"+link.a.get('href')


def init(config):
    return Vier("vier", config["vier"]["download_url"], config["vier"]['overview_url'],
                config["vier"]['login']['url'], config["vier"]['login']['username'],
                config["vier"]['login']['password'])